/**
 * @file vtol_type.cpp
 * @brief VtolType class implementation for VTOL (Vertical Takeoff and Landing) vehicle control.
 *
 * This class implements the core functionality for controlling the VTOL mode transitions,
 * attitude control, and state updates for a tail-sitter VTOL drone.
 */
...
/**
 * @brief Constructor for the VtolType class.
 * @param att_controller Pointer to VtolAttitudeControl object.
 */
...
/**
 * @brief Initializes the VtolType object.
...
/**
 * @brief Updates the parameters for VTOL control.
 * This ensures that transition speeds and open-loop transition times
...
/**
 * @brief Updates the state in multicopter mode.
 * Resets attitude and weight for multicopter control.
 */
...
/**
 * @brief Updates the state in fixed-wing mode.
 * Resets integrators and handles transitions to fixed-wing mode.
 */
...
/**
 * @brief Updates the state during the transition phase.
 *
 * Updates timestamps, handles transition timing, checks for quadchute conditions,
 * and adjusts throttle blending.
 */
...
/**
 * @brief Check if the front transition to fixed-wing is completed.
 *
 * This function evaluates whether the transition from multirotor mode to fixed-wing mode
 * is completed by monitoring parameters such as airspeed, transition time, and
 * ground conditions.
 *
 * @return true if the front transition is completed, false otherwise.
 */
...
/**
 * @brief Base logic for checking the completion of the front transition.
 *
 * This function checks the airspeed, minimum and open-loop transition times to determine
 * if the transition to fixed-wing mode can be completed. It can be overridden by derived classes
 * for custom transition logic.
 *
 * @return true if transition criteria are met, false otherwise.
 */
...
/**
 * @brief Check if the vehicle can transition on the ground.
 *
 * This function checks whether the vehicle is either disarmed or landed,
 * preventing transition in these cases.
 *
 * @return true if the vehicle can transition while on the ground, false otherwise.
 */
...
/**
 * @brief Reset transition states.
 *
 * This function resets transition-related states, including the start timestamp
 * and the local position at the start of the transition. It is called when a new
...
/**
 * @brief Check if quadchute is enabled.
 *
 * This function checks if the vehicle is above the quadchute altitude limit and if the quadchute
 * feature is armed and enabled, indicating that a quadchute maneuver might be needed.
 *
 * @return true if quadchute is enabled, false otherwise.
 */
...
/**
 * @brief Check if the minimum altitude has been breached.
 *
 * This function checks whether the vehicle's current altitude is below the
 * minimum altitude defined for fixed-wing flight, potentially triggering a quadchute.
 *
 * @return true if the minimum altitude has been breached, false otherwise.
 */
...
/**
 * @brief Check for uncommanded descent.
 *
 * This function checks if the vehicle is descending without command during
 * fixed-wing flight and if the descent exceeds the configured altitude loss threshold.
 *
 * @return true if the vehicle is in an uncommanded descent, false otherwise.
 */
...
/**
 * @brief Check for altitude loss during front transition.
 *
 * This function monitors altitude loss during the front transition to fixed-wing mode,
 * comparing the current altitude with the altitude at the start of the transition.
 *
 * @return true if the altitude loss exceeds the configured threshold, false otherwise.
 */
...
/**
 * @brief Handle resets from the Extended Kalman Filter (EKF).
 *
 * This function adjusts the transition-related state variables, including altitude,
 * if a reset in the z-direction is detected from the EKF.
 */
...
/**
 * @brief Check if the pitch angle exceeds the allowed maximum.
 *
 * This function checks if the vehicle's pitch angle during fixed-wing flight exceeds
 * the maximum pitch angle allowed by the parameters.
 *
 * @return true if the maximum pitch is exceeded, false otherwise.
 */
...
/**
 * @brief Check if the roll angle exceeds the allowed maximum.
 *
 * This function checks if the vehicle's roll angle during fixed-wing flight exceeds
 * the maximum roll angle allowed by the parameters.
 *
 * @return true if the maximum roll is exceeded, false otherwise.
 */
...
/**
 * @brief Check for a front transition timeout.
 *
 * This function checks if the front transition from multirotor to fixed-wing mode
 * has timed out, and if so, it aborts the transition.
 *
 * @return true if the transition timeout is reached, false otherwise.
 */
...
/**
 * @brief Get the reason for triggering a quadchute.
 *
 * This function evaluates various conditions, such as altitude loss, uncommanded descent,
 * or exceeding pitch or roll limits, and returns the reason for activating the quadchute.
 *
 * @return The reason for triggering a quadchute.
 */
...
/**
 * @brief Handle external quadchute commands.
 *
 * This function processes any special external commands to trigger a quadchute,
 * typically received during emergency transitions.
 */
...
/**
 * @brief Check for conditions that require a quadchute.
 *
 * This function evaluates all quadchute conditions, such as altitude loss, pitch/roll
 * exceeding limits, or external commands, and triggers the quadchute if necessary.
 */
...
/**
 * @brief Calculate pusher assist throttle.
 *
 * This function calculates the amount of pusher throttle to assist forward flight during
 * transitions, depending on altitude, landing conditions, and other parameters.
 *
 * @return The amount of forward thrust provided by the pusher motor.
 */
