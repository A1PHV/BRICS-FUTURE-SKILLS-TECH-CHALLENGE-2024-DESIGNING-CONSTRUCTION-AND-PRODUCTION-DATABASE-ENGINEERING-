...
@file tiltrotor.cpp
@brief Tiltrotor VTOL type implementation.
This file contains the control logic for a tiltrotor VTOL aircraft, which transitions between multicopter
mode and fixed-wing mode by tilting its rotors. The transitions are controlled by various parameters
and conditions such as speed, time, and tilt angle. */
...
@brief Constructor for the Tiltrotor class.
Initializes the tiltrotor control system by calling the base class constructor.
@param attc Pointer to VtolAttitudeControl object. */
...
@brief Updates the tiltrotor parameters.
Updates the VTOL parameters from the parameter store. */
...
@brief Updates the VTOL state.

Controls the state transitions between multicopter (MC) and fixed-wing (FW) modes,

based on conditions such as user input, speed, and tilt angles. It also handles

failsafe events that force immediate transitions. */
...
@brief Updates the multicopter state.

Adjusts the control values specific to multicopter mode, such as yaw control and tilt angle. */
...
@brief Updates the fixed-wing state.

Adjusts the control values specific to fixed-wing mode, such as throttle and tilt angle. */
...
@brief Updates the transition state between MC and FW modes.

Controls the transition process, including tilt angles, thrust adjustments, and blending of control weights. */
...
@brief Waits for the TECS (Total Energy Control System) to provide data.
During transitions, the aircraft will maintain multicopter thrust until data from TECS is available. */
...
@brief Fills actuator outputs with control values.

Publishes torque and thrust setpoints for both multicopter and fixed-wing motors, as well as tiltrotor-specific controls. */
...
@brief Blends throttle during a back transition.
@param scale Scaling factor between 0 and 1.
@param target_throttle The target throttle to blend towards. */
...
@brief Returns the time until motors are fully tilted back during back transition.
